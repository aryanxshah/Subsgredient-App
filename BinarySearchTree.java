...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        if (!isChildOfParent(child, parent)) {
        ...
        if (parent.down[1] == child) {
            newParent = rotateRight(child, parent);
        } else if (parent.down[0] == child) {
            newParent = rotateLeft(child, parent);
        }

        updateRootIfNeeded(parent, newParent);

    }

    private Node<T> rotateRight(Node<T> child, Node<T> parent) {
        // Rotate right logic
    ...
    private Node<T> rotateLeft(Node<T> child, Node<T> parent) {
        // Rotate left logic
    ...
    private void updateRootIfNeeded(Node<T> oldParent, Node<T> newParent) {
        if (oldParent == root) {
            root = newParent;
        } else {
            if (oldParent.up != null && oldParent.up.down[0] == oldParent ) {
                oldParent.up.down[0] = newParent;
            } else {
                oldParent.up.down[1] = newParent;
            }
        }
    }

    boolean isChildOfParent(Node<T> child, Node<T> parent) {
        return parent.down[0] == child || parent.down[1] == child;
    }
    /**
    ...
    public static boolean test1() {
        ...
        // Test left rotation
        // Construct a tree with the following structure:
        ...
        // Perform a left rotation at the root (5)
        // The tree should be rotated to:
    ...
    public static boolean test2() {
        ...
        // Perform a right rotation at the root (3)
    ...
    public static boolean test3() {
        // TODO: Implement this test.
        // Test rotation at a node that is NOT a root
        ...
        // Perform a right rotation at node #3 (parent: 5, child: 3)


        tree.rotate(tree.root.down[0].down[1], tree.root.down[0]);
